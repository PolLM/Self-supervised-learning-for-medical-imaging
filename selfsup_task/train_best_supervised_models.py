...
def supervised_train(config, model_type, i=0):
    '''
    #################################################################################################
    Supervised training
    ...
    folder_logs = f"runs\\final_trainings\\4_final_trainings\\train_best_supervised_cropped_8e6\\{model_type}_iter_{i}_crop_{cropping}"
    ...
    elif config["model_path"] == "pretrained":
        model = torchvision.models.resnet18(zero_init_residual=True, pretrained=True)
        model.conv1 = nn.Conv2d(1, 64, 7, 2, 3, bias=False)
    else:
        model = torchvision.models.resnet18(zero_init_residual=True)
        model.conv1 = nn.Conv2d(1, 64, 7, 2, 3, bias=False)
...
for j, model_path in enumerate(pretrained_models):
    if j==0:
        ...
        model_type = "Imagenet_pretrained"
    else:
    ...
    config = {  "project_path": PROJECT_PATH,
        ...
        "transforms_prob": 0.5,
        "h_flip": 1, 
        "optimizer": 0,#"Adam", #Adam, SDG
        "optimizer_weight_decay": 1e-4,#1e-5, # 0, 1e-4
        "soft_crop": 240,#250, # o, 250
        "lr_sup": 8e-6,#8e-4,#1e-4, # 1e-3, 1e-5
        "batch_size_sup": 96,#64, # 32,64,128
        }
    supervised_train(config, model_type)