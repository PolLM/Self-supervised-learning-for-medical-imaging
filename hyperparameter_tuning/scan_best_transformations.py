...
from utils.training_utils import train_one_epoch,eval_one_epoch
...
if config["mode"] == "scan_transforms":
    ...
    for extra_T_name, extra_T in extra_transforms.items():
        ...
        checkpoints_path = os.path.join(PROJECT_PATH, f"runs/transforms/{extra_T_name}_linear_prediction")
        writer = SummaryWriter(checkpoints_path)
        #Resetting seeds to have the same initial model parameters
        torch.cuda.empty_cache()
        random.seed(config["random_seed"])
        ...
        test_len = int(config["test_frac"]*len(dataset))
        ...
        for epoch in range(config["num_epochs"]):    
            ...
            losses, targets, predictions = train_one_epoch(train_loader, model, optimizer, config, criterion=criterion, return_targets=True)
            losses_eval, targets_eval, predictions_eval = eval_one_epoch(train_loader, model, optimizer, config, criterion=criterion, return_targets=True)
            scheduler.step()
