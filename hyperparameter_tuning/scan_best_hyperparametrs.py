...
def self_supervised_train(config, barlow_lambda, projector_dims):
    ...
    folder_name = str(barlow_lambda) + projector_sizes + str(config["optimizer_weight_decay"]) + "2nd_iter"
...
if config["mode"] == 'self_supervised':
    ...
    BASE_DIR = os.path.join(PROJECT_PATH, f"runs/hyperparams")
    for root, subdirectories, files in os.walk(BASE_DIR):
        for subdirectory in subdirectories:
            if "ACC" in subdirectory or "2nd_iter" not in subdirectory:# or "0.0001_512__512__512__512_1e-05" in subdirectory:
                continue
            ...
            checkpoints_path = os.path.join(root, subdirectory + "ACC_full_2nd_iter_prediction")
